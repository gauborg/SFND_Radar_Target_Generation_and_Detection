...
for i=1:length(t)         
	...
	td(i) = 2 * r_t(i)/c;								% range_changes every t(i) instant
...
%signal_fft = signal_fft./Nr;	%normalizing here...
...
figure('Name', '2nd FFT output');
surf(doppler_axis,range_axis,RDM);
title('3D output of 2DFFT');
xlabel('Velocity (m/s)');
ylabel('Range (m)');
zlabel('Amplitude');
...
offset = 1.4;   %calculated this value approximately by diving signal with the noise...
...
guard_cells = (2*Gr+1)*(2*Gd+1)-1;
disp(total_cells);
disp(guard_cells);
training_cells1 = ((2*Tr+2*Gr+1)*(2*Td+2*Gd+1))-((2*Gr+1)*(2*Gd+1));
total_tr = (2*(Td+Gd+1)*2*(Tr+Gr+1))-((Gr*Gd)-1);
disp(training_cells1);
disp(total_tr);
...
for i = Tr+Gr+1:(Nr/2)-(Tr+Gr)
    for j = Td+Gd+1:(Nd)-(Td+Gd)
        ...
        % Add the signal level within each training cell to the noise...
        ...
        %converting back from power to db...
        threshold = pow2db(noise_level/total_tr);
